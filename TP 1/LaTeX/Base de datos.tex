...
\tadIgualdadObservacionalSimple{($\forall$ $b_1$,$b_2$:bdd) ($b_1$ $\igobs$ $b_2$) $\Leftrightarrow$ \Bigg( tablas($b_1$) $\igobs$ tablas($b_2$) $\wedge$ (($\forall$ $t_1$,$t_2$:tabla)($\{$ $t_1$, $t_2$ $\}$ $\in$ tablas($b_1$) $\wedge$ $\{$ $t_1$, $t_2$ $\}$ $\in$ tablas(b2)) camposDeJoin($t_1$,$t_2$,$b_1$) $\igobs$ camposDeJoin($t_1$,$t_2$,$b_2$) $\wedge$  $\qquad$ QueTriggerea?($t_1$,$b_1$) $\igobs$ QueTriggerea?($t_1$,$b_1$)) $\wedge$ ($\forall$ $t_3$: tabla) $t_3$ $\in$ tablas($b_1$) $\impluego$ (\#Modificaciones($t_3$, $b_1$) $\igobs$ \#Modificaciones($t_3$, $b_2$)) \Bigg)}
...
\tadAxioma{MasMod($ts$, $b$)}{\IF $\emptyset$?(sinUno(ts)) THEN sinUno(ts) ELSE {\IF \#Modificaciones(DameUno(ts), b) $\geqslant$ \#Modificaciones(MasMod(sinUno(ts), b)) THEN DameUno(ts) ELSE MasMod(SinUno(ts, b))FI} FI}
\bigskip
