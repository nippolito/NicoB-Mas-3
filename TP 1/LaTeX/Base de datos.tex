...
\tadAxioma{MasMod($ts$, $b$)}{\IF $\emptyset$?(ts) THEN NuevaTabla ELSE {\IF \#Modificaciones(DameUno(ts), b) $\geqslant$ \#Modificaciones(MasMod(sinUno(ts), b)) THEN \#Modificaciones(DameUno(ts, b)) ELSE MasMod(SinUno(ts, b))FI} FI}
\bigskip
