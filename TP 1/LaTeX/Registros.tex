...
\tadOperacion{DameCamposCoinc}{reg $\quad$ r, conj(campo) $\quad$ ci}{conj(reg)}{($\forall$ r : reg) r $\in$ $\impluego$ ci $\in$ campos(r)}

\tadOperacion{RearmarCoinci}{reg $\quad$ r, conj(campo) $\quad$ ci}{reg}{ci $\in$ campos(r)}

\tadOperacion{Busca}{conj(campo) $\quad$ cs , reg $\quad$ r, conj(reg) $\quad$ rs, conj(campo) $\quad$ ci}{conj(reg)}{ cs $\subseteq$ campos(r) $\wedge$ $\neg$ ($\emptyset$?(cs)) $\wedge$ ci $\subseteq$ cs }
...
\tadAxioma{DameCamposCoinci(r,ci)}{\IF $\emptyset$?(ci) THEN $\emptyset$ ELSE Ag(RearmarCoinci(DameUno(cs),ci),DameCamposCoinci(SinUno(rs),ci) FI}
...
\tadAxioma{Busca(cs,r,rs,ci)}{\IF $\emptyset$?(SinUno(cs)) THEN {\IF tipo(DameUno(cs)) = Nat THEN DameCoincidentesVal(DameUno(cs),valorEn(r,DameUno(cs)),rs) ELSE DameCoincidentesPal(DameUno(cs),valorEn(r,DameUno(cs)),rs) FI} ELSE {\IF tipo(DameUno(cs)) = Nat THEN DameCoincidentesVal(DameUno(cs),valorEn(r,DameUno(cs)),Busca(SinUno(cs),r,rs)) ELSE DameCoincidentesPal(DameUno(cs),valorEn(r,DameUno(cs)),Busca(SinUno(cs),r,rs)) FI} FI}
