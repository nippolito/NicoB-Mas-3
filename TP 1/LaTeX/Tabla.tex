...
\tadOperacion{Campos}{tabla}{conj(campo)}{}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\tadOperacion{DefClave}{tabla $\quad$ t, campo $\quad$ c}{tabla}{c $\in$ campos(t) $\wedge$ ($\forall$ $r_1$, $r_2$ : registro)( $\neg$($r_1$ $\igobs$ $r_2$) $\wedge$ $r_2$ $\in$ registros(t)) $\impluego$ noRepiten($r_1$, $r_2$, c)}
\tadOperacion{A\~nadirReg}{a}{b}{tabla $\quad$ t, reg $\quad$ r}{tabla}{(campos(r) $\igobs$ campos(t)) $\wedge$ ($\forall$ c : campo)(c $\in$ claves(t) $\impluego$ ($\forall$ $r_1$ : registro) ($\neg$($r_1$ $\igobs$ r) $\wedge$ $r_1$ $\in$ registros(t)) $\impluego$ noRepiten(r, $r_1$, c)))}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\tadOtrasOperaciones
